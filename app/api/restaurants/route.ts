...
// Better neighborhood detection from coordinates
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (restaurantId) {
      ...
      if (data) {
        // Include ALL data fields and correct neighborhood
    ...
    // Allow up to 100 results per search to show more restaurants
    const actualLimit = Math.min(limitNum, 100)
    
    // FIXED: Remove nullsLast parameters that break Render deployment
    ...
    const transformedData: TransformedRestaurant[] = data?.filter(restaurant => {
      ...
      // Correct neighborhood detection using coordinates
