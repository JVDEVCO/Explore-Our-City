...
async function importMiamiRestaurants() {
    console.log('=== IMPORTING MIAMI BEACH RESTAURANTS ===\n')

    const baseUrl = 'http://www.miamibeachapi.com/rest/a.pi/businesses/search'

    try {
        // Get restaurants from both categories (361 = restaurant-bars, 383 = food-sales)
        const response = await fetch(`${baseUrl}?category_filter=361,383&limit=100`)
        ...
        for (const business of data.businesses) {
            try {
                // Map price range from API data (if available)
                ...
                if (business.datatables?.['restaurant-bars']?.price_range_restaurant) {
                    ...
                    if (apiPrice.includes('inexpensive') || apiPrice.includes('cheap')) priceRange = '$'
                    else if (apiPrice.includes('moderate')) priceRange = '$$'
                    else if (apiPrice.includes('expensive') || apiPrice.includes('upscale')) priceRange = '$$$'
                    else if (apiPrice.includes('very expensive') || apiPrice.includes('luxury')) priceRange = '$$$$'
                }
                ...
                // Get phone number from restaurant data
                const phone = business.datatables?.['restaurant-bars']?.telephone || business.phone || null
